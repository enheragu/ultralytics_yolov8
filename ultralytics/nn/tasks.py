...
from ultralytics.nn.modules import (AIFI, C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    ...
                                    RTDETRDecoder, Segment, FilterInput, FilterInputDetach)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    if verbose:
        LOGGER.info(f"\n{'':>3}{'from':>20}{'n':>3}{'params':>10}  {'module':<45} {'input ch':<10} {'output ch':<10} {'arguments':<30}")
    ch = [ch]
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        ## EEHA - Number of channels from configured args
        elif m is FilterInput:
            c2 = len(args) if isinstance(args, (list, tuple)) else 1  # number of channels
            c1 = ch[f]  # input channels
        elif m is FilterInputDetach:
            c2 = ch[f[0]]
            c1 = ch[f[0]]
        else:
            c2 = ch[f]
        ...
        if 'c1' in locals():
            c1_log = f'{c1}'
        if verbose:
            LOGGER.info(f'{i:>3}{str(f):>20}{n_:>3}{m.np:10.0f}  {t:<45}{c1_log:<10}{c2:<10}{str(args):<30}')  # print
...
def guess_model_scale(model_path):
    ...
    with contextlib.suppress(AttributeError):
        import re
        return re.search(r'\d+([nslmx])', Path(model_path).stem).group(1)  # n, s, m, l, or x
    return ''
